...
public class AppUtils {
    ...
    public static boolean isvalidifsc(String ifsccode) {
        String ifsc = "[A-Z|a-z]{4}[0][a-zA-Z0-9]{6}$";
        Pattern pattern = Pattern.compile(ifsc);
        Matcher matcher = pattern.matcher(ifsccode);
        return matcher.matches();
    }

    public static boolean isvalidaccountnumber(String accountnumber) {
        String accountnumber1 = "^d{9,18}$";
        Pattern pattern = Pattern.compile(accountnumber1);
    ...
    public static void centerToolbarTitle(@NonNull Toolbar toolbar, @NonNull Activity mActivity) {
        ...
        if (!outViews.isEmpty()) {
            ...
            ((Toolbar.LayoutParams) titleView.getLayoutParams()).width = -1;
    ...
    void changeProgressbarColor(ProgressBar mProgressBar, Activity mActivity) {
        if (Build.VERSION.SDK_INT < 21) {
            ...
            DrawableCompat.setTint(wrapDrawable, ContextCompat.getColor(mActivity, R.color.red));
            mProgressBar.setIndeterminateDrawable(DrawableCompat.unwrap(wrapDrawable));
            return;
        }
        mProgressBar.getIndeterminateDrawable().setColorFilter(ContextCompat.getColor(mActivity, R.color.red), PorterDuff.Mode.SRC_IN);
    }
